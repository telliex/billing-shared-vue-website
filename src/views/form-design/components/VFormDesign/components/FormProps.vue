...
<template>
  <div class="properties-content">
    <Form class="properties-body" label-align="left" layout="vertical">
      ...
      <!-- </Row> -->
      <FormItem label="標籤寬度（px）" v-show="formConfig.labelLayout === 'flex'">
        <InputNumber
          :style="{ width: '100%' }"
      ...
      <div v-if="formConfig.labelLayout === 'Grid'">
        ...
        <FormItem label="控件大小">
          <RadioGroup button-style="solid" v-model:value="formConfig.size">
            <RadioButton value="default">默認</RadioButton>
      ...
      <FormItem label="表單屬性">
        <Col
          ><Checkbox v-model:checked="formConfig.colon" v-if="formConfig.layout == 'horizontal'"
            >label後面顯示冒號</Checkbox
          ></Col
        >
        <Col><Checkbox v-model:checked="formConfig.disabled">禁用</Checkbox></Col>
        <Col><Checkbox v-model:checked="formConfig.hideRequiredMark">隱藏必選標記</Checkbox></Col>
      </FormItem>
    </Form>
  </div>
</template>
