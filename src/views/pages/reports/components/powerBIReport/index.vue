...
<script lang="ts" setup name="PowerBIReport">
  ...
  let currentPageReportName = ref(props.currentPageReportName);
  ...
  async function embedReport(): Promise<void> {
    ...
    let tempRes: any = await axios({
      ...
      url: powerBILambdaURL.value,
      data: {
        userId: currentUserId,
        pageName: currentPageReportName.value,
        pageIdKey: currentPageReportIdKey.value,
      },
    }).catch((err) => {
  ...
  async function setReportObj(value: ReportExtended) {
    ...
    let tableName = await GetDictionaryItems({
      ...
      request_items: [
        {
          item_type: 'PowerBI',
          item_key: currentPageTableNameKey.value,
          item_key2: '',
        },
      ],
    });
