...
<script lang="ts" setup name="Mpdf">
  ...
  let tableName = ref(t('report.mpdf.tableAreaTitle'));
  let reportType = ref('M0100'); // [M] report type & S3 prefix folder name,
  let s3Bucket = import.meta.env.VITE_GLOB_S3_INVOICE_REPORT; // [M] S3 bucket name
  ...
  function handleChildFormValue(values: SearchItems) {
    ...
    let S3FileName = `${fileMonth}-${S3ReportClass}-861-00000100-1.pdf`; // [for need to modify area] // [for need to modify area]
    objectKeyString.value = `bill_invoice_raw/bill_invoice_output_desc_${fileMonth}/${S3FileName}`;
