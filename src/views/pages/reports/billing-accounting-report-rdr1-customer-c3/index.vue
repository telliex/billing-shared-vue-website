...
<script lang="ts" setup name="CustomerRDR1C3">
  ...
  let tableName = ref(t('report.customerrdr1c3.tableAreaTitle'));
  let reportType = 'c3'; // report type & S3 prefix folder name,
  ...
  async function handleSearchSubmit(values: SearchItems) {
    ...
    let S3FileName = `${S3ReportClass}_${dayjs(values.YearMonth).format('YYYYMM').toString()}_30_0_migration_opor.csv`;
    ...
    let S3Location = await GetS3TargetUrl({
      ...
      object_key: `ym=${S3Year}${S3Month}/leadger_country=${S3ReportClass}/report_type=0/${S3FileName}`,
      duration: '10',
    }).catch((err) => {
      console.log(err);
