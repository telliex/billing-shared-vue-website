...
<script lang="ts" setup name="CreditownerReportaccountant">
  ...
  let tableName = ref(t('report.creditownerReportaccountant.tableAreaTitle'));
  let reportType = 'billing_credit_report_accountant'; // report type & S3 prefix folder name,
  ...
  function exportFile() {
    ...
    jsonToSheetXlsx({
      ...
      filename: `credit_accountant_${timeStamp}.xlsx`,
      write2excelOpts: {
        bookType: 'xlsx'
      },
    });
...
<script lang="ts">
  export default {
    name: 'CreditownerReportaccountant',
  };
</script>
