...
<script lang="ts" setup name="CreditownerReportaccountant">  //[for need to modify area]
  ...
  let tableName = ref(t('report.creditownerReportsales.tableAreaTitle')); //[for need to modify area]
  let reportType = ref('billing_credit_report_sales'); // [M] report type & S3 prefix folder name,
  ...
  function handleChildFormValue(values: SearchItems) {
    ...
    let fileMonth = dayjs(values.YearMonth).format('YYYYMMDD').toString();// [for need to modify area]
...
">  //[for need to modify area]
  ...
  let tableName = ref(t('report.creditownerReportaccountant.tableAreaTitle')); //[for need to modify area]
  let reportType = ref('billing_credit_report_accountant'); // [M] report type & S3 prefix folder name,
  let s3Bucket = import.meta.env.VITE_GLOB_S3_REPORT; // [M] S3 bucket name
