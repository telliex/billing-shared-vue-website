...
<script lang="ts" setup name="BillingriListCR">
  ...
  let tableName = ref(t('report.billingrilistcr.tableAreaTitle'));
  ...
  function exportFile() {
    ...
    jsonToSheetXlsx({
      ...
      filename: `ri_list_cr_${timeStamp}.xlsx`,
      write2excelOpts: {
        bookType: 'xlsx',
      },
    });
  ...
  async function handleSearchSubmit(values: SearchItems) {
    ...
    let S3FileName = `${S3ReportClass}_${dayjs(values.YearMonth).format('YYYYMM').toString()}_6_sap_ri_list.csv`;
