...
<script lang="ts" setup name="BillingriUsageCR">
  ...
  let tableName = ref(t('report.billingriusagecr.tableAreaTitle'));
  ...
  function exportFile() {
    ...
    jsonToSheetXlsx({
      ...
      filename: `ri_usage_cr_${timeStamp}.xlsx`,
      write2excelOpts: {
        bookType: 'xlsx',
      },
    });
  ...
  async function handleSearchSubmit(values: SearchItems) {
    ...
    let S3FileName = `${S3ReportClass}_${dayjs(values.YearMonth).format('YYYYMM').toString()}_6_sap_ri_usage.csv`;
    ...
    let S3Location = await GetS3TargetUrl({
      ...
      object_key: `ym=${S3Year}${S3Month}/leadger_country=${S3ReportClass}/report_type=6/${S3FileName}`,
      duration: '10',
    }).catch((err) => {
      console.log(err);
